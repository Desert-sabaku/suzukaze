...
func main() {
	...
	go heartbeat(led, 500*time.Millisecond)
	go readMessages()

	select {}
}

// readMessages reads newline-delimited JSON Messages from USB serial
// (machine.Serial) and dispatches each one to the fade worker for its pin.
func readMessages() {
	line := make([]byte, 0, maxLineSize)
	...
	for {
		...
		case b == '\n':
			if len(line) > 0 {
				...
				if err := json.Unmarshal(line, &msg); err != nil {
					log.Error().Err(err).Str("line", string(line)).Msg("Failed to parse message")
				} else {
					dispatch(msg)
				}
			}
		...
		case b == '\r':
			// ignore
		case len(line) >= maxLineSize:
			// line too long without a newline; drop it and resync
			line = line[:0]
		default:
			line = append(line, b)
		}
